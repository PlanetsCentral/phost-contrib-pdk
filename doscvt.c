...
static Uns16 ReadDOSUns16(const void *pVal)
{
#ifdef __MSDOS__
    return *(Uns16 *)pVal;
#else
...
static Uns32 ReadDOSUns32(const void *pVal)
{
#ifdef __MSDOS__
    return *(Uns32 *)pVal;
#else
...
{
#ifdef __MSDOS__
    if (pNum NEQ fread(pData, 2, pNum, pInFile)) return False;
#else
    unsigned char lData[2];
    while (pNum--) {
        if (2 NEQ fread(lData, 1, 2, pInFile)) return False;
...
{
#ifdef __MSDOS__
    if (pNum NEQ fread(pData, 4, pNum, pInFile)) return False;
#else
    unsigned char lData[4];

    while (pNum--) {
        if (4 NEQ fread(lData, 1, 4, pInFile)) return False;
...
Boolean DOSWrite16(const Uns16 *pData, Uns16 pNum, FILE *pOutFile)
{
#ifdef __MSDOS__
    if (pNum NEQ fwrite(pData, 2, pNum, pOutFile)) return False;
#else
    Uns16 lVal;
    while (pNum--) {
        lVal = *pData++;
        if (EOF EQ fputc((lVal & 0xFFU), pOutFile)) return False;
        if (EOF EQ fputc((lVal >> 8) & 0xFFU, pOutFile)) return False;
    }
#endif
    return True;
}

Boolean DOSWrite32(const Uns32 *pData, Uns16 pNum, FILE *pOutFile)
{
#ifdef __MSDOS__
    if (pNum NEQ fwrite(pData, 4, pNum, pOutFile)) return False;
#else
    Uns32 lVal;
    while (pNum--) {
        lVal = *pData++;
        if (EOF EQ fputc((int)(lVal & 0xFFU), pOutFile)) return False;
        if (EOF EQ fputc((int)((lVal >> 8) & 0xFFU), pOutFile)) return False;
        if (EOF EQ fputc((int)((lVal >> 16) & 0xFFU), pOutFile)) return False;
        if (EOF EQ fputc((int)((lVal >> 24) & 0xFFU), pOutFile)) return False;
    }
#endif
    return True;
}
...
void EndianSwap32(void *pData, Uns16 pNumber)
{
...
void EndianSwap16(void* pData, Uns16 pNumber)
{
#ifdef BIG_ENDIAN   /* NEW */
    Uns16 lVal;
    while (pNumber-- > 0)
...
Boolean DOSReadStruct(const DOSConvertElement *pStruct, Uns16 pNumElements,
                            void *pData, FILE *pInFile)
{
    ...
    for (lCount = 0; lCount < pNumElements; lCount++) {
        ...
        switch (pStruct[lCount].mType) {
         case DOSCVT_char:
            if (1 NEQ fread(lData, pStruct[lCount].mSize, 1, pInFile)) goto bad_read;
            break;

         case DOSCVT_Uns16:
            if (! DOSRead16((Uns16 *)lData, 1, pInFile)) goto bad_read;
            break;

         case DOSCVT_Uns32:
            if (! DOSRead32((Uns32 *)lData, 1, pInFile)) goto bad_read;
            break;
            ...
            switch (sizeof(DOSCVT_Def)) {
             ...
             case 4:
                /* Must convert 2-byte DOS word to 4-byte enum on our machine */
                {
                    ...
                    if (! DOSRead16(&lEnumVal, 1, pInFile)) goto bad_read;
...
Boolean DOSWriteStruct(const DOSConvertElement *pStruct, Uns16 pNumElements,
                            const void *pData, FILE *pOutFile)
{
    ...
    for (lCount = 0; lCount < pNumElements; lCount++) {
        ...
        switch (pStruct[lCount].mType) {
         case DOSCVT_char:
            if (1 NEQ fwrite(lData, pStruct[lCount].mSize, 1, pOutFile)) goto bad_write;
            break;

         case DOSCVT_Uns16:
            if (! DOSWrite16((Uns16 *)lData, 1, pOutFile)) goto bad_write;
            break;

         case DOSCVT_Uns32:
            if (! DOSWrite32((Uns32 *)lData, 1, pOutFile)) goto bad_write;
            break;

         case DOSCVT_enum:
            /* On our machine, enums had better be word-sized, like DOS,
               or longword-sized, like most 32-bit machines */
            switch (sizeof(DOSCVT_Def)) {
             case 2:
                if (! DOSWrite16((Uns16 *)lData, 1, pOutFile)) goto bad_write;
                break;
             case 4:
                /* Must convert 4-byte enum on our machine to 2-byte enum
                   in DOS */
                {
                    Uns16 lEnumVal = (Uns16) (*(DOSCVT_Def *)lData);
                    if (! DOSWrite16(&lEnumVal, 1, pOutFile)) goto bad_write;
                }
                break;
             default:
                passert(0);
...
const Uns16 NumShipStruct_Convert = sizeof(ShipStruct_Convert)/sizeof(ShipStruct_Convert[0]);
...
const Uns16 NumPlanetStruct_Convert = sizeof(PlanetStruct_Convert)/sizeof(PlanetStruct_Convert[0]);
...
const Uns16 NumBaseStruct_Convert = sizeof(BaseStruct_Convert)/sizeof(BaseStruct_Convert[0]);
...
const Uns16 NumMineStruct_Convert = sizeof(MineStruct_Convert)/sizeof(MineStruct_Convert[0]);
...
const Uns16 NumHullspecStruct_Convert = sizeof(HullspecStruct_Convert)/sizeof(HullspecStruct_Convert[0]);
...
const Uns16 NumTurntimeStruct_Convert = sizeof(TurntimeStruct_Convert)/sizeof(TurntimeStruct_Convert[0]);
...
const Uns16 NumBaseOrderStruct_Convert = sizeof(BaseOrderStruct_Convert)/sizeof(BaseOrderStruct_Convert[0]);
...
const Uns16 NumHostGenStruct_Convert = sizeof(HostGenStruct_Convert)/sizeof(HostGenStruct_Convert[0]);
...
const Uns16 NumEngspecStruct_Convert = sizeof(EngspecStruct_Convert)/sizeof(EngspecStruct_Convert[0]);
...
const Uns16 NumBeamspecStruct_Convert = sizeof(BeamspecStruct_Convert)/sizeof(BeamspecStruct_Convert[0]);
...
const Uns16 NumTorpspecStruct_Convert = sizeof(TorpspecStruct_Convert)/sizeof(TorpspecStruct_Convert[0]);
...
const Uns16 NumXyplanStruct_Convert = sizeof(XyplanStruct_Convert)/sizeof(XyplanStruct_Convert[0]);
...
const Uns16 NumShipxyStruct_Convert = sizeof(ShipxyStruct_Convert)/sizeof(ShipxyStruct_Convert[0]);
...
const Uns16 NumVcrStruct_Convert = sizeof(VcrStruct_Convert)/sizeof(VcrStruct_Convert[0]);
...
const Uns16 NumTargetShipStruct_Convert = sizeof(TargetShipStruct_Convert)/sizeof(TargetShipStruct_Convert[0]);
...
const Uns16 NumExtMessageStruct_Convert = sizeof(ExtMessageStruct_Convert)/sizeof(ExtMessageStruct_Convert[0]);
...
const Uns16 NumUFOStruct_Convert = sizeof(UFOStruct_Convert)/sizeof(UFOStruct_Convert[0]);
