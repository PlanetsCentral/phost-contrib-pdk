...
IO_Def Read_Turntime_File(void)
{
    FILE            *lTurntimeFile;
    ...
    if ((lTurntimeFile = OpenInputFile(NEXTTURN_FILE, GAME_DIR_ONLY | NO_MISSING_ERROR)) NEQ NULL) {
            ...
            if (! DOSReadStruct(TurntimeStruct_Convert,
							...
							lTurntimeFile)) {
#endif
            /* If nextturn.hst exists but is 0 length, then this is a
               newly MASTER-ed game */
            gNewlyMastered = True;
        }
        fclose(lTurntimeFile);
    } else gNewlyMastered = True;
...
const Turntime_Struct *RawTurnTime(void)
{
    return &gTurntime;
}
