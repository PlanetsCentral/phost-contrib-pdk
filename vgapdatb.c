...
InitXY(void)
{
  if (gXYPlanPtr EQ 0) {
    gXYPlanPtr =
          (XYData_Struct *) MemCalloc(PLANET_NR + 1, sizeof(XYData_Struct));
...
Read_Xyplan_File(void)
{
  ...
  if ((XYPlanFile =
              OpenInputFile(XYPLAN_FILE,
              GAME_OR_ROOT_DIR | NO_MISSING_ERROR)) NEQ NULL) {
#ifdef __MSDOS__
    if (PLANET_NR NEQ fread(gXYPlanPtr + 1, sizeof(XYData_Struct), PLANET_NR,
                XYPlanFile)) {
