...
typedef struct {
    ...
    Uns16   mPlanetCoreRangesUsual[8];   /* Nmin, Tmin, Dmin, Mmin, Nmax, Tmax, Dmax, Mmax */
    Uns16   mPlanetCoreRangesAlternate[8];   /* as above */
    Uns16   mPlanetCoreUsualFrequency[4];  /* Nfreq, Tfreq, Dfreq, Mfreq */
    Uns16   mPlanetSurfaceRanges[8];     /* Nmin, Tmin, Dmin, Mmin, Nmax, Tmax, Dmax, Mmax */
    Uns16   mPlanetDensityRanges[8];     /* Nmin, Tmin, Dmin, Mmin, Nmax, Tmax, Dmax, Mmax */
...
typedef struct {
    ...
    Uns16   mScenario;      /* 1-Classic, 3-Wandering, 6-Ashes, 7-Crazy, 8-Disunited */
    Uns16   mHomeworlds[12];
    Uns16   mFlagPoints[12]; /* # of turns a race has controlled flag world */
    ...
    Uns16   mGameGoal;       /* Goal of the game:
...
DoAssignment(const char *name, char *val)
{
	...
	for (ix=0; ix<CF_NumItems; ix++) {
		if (!stricmp(Names[ix], name)) break;
	}
	if (ix >= CF_NumItems) {
	...
	switch (Types[ix]) {
	 case CFType_Uns8: return readUns8(ix, val);
	 case CFType_Uns16: return readUns16(ix, val);
	 case CFType_Int16: return readInt16(ix, val);
	 case CFType_Uns32: return readUns32(ix, val);
	 case CFType_Boolean: return readBooleanType(ix, val);
     case CFType_Float: return readFloatType(ix, val);
     case CFType_String: return readStringType(ix, val);
     case CFType_List: return readListType(ix, val);
	 default:
		passert(0);
	}
...
readBooleanType(Uns16 ix, char *val)
{
	...
	for (i=0; i<n; i++) {
		...
		if (match > 1) match -= 2;	/* Convert no/yes to false/true */
...
readFloatType(Uns16 ix, char *val)
{
	...
	for (i=0; i<n; i++) {
        if (!getDouble(val, &fval)) return 0;
		val = 0;

        if (!checkValue(ix, fval)) return 0;
...
processOptions(int argc, char *argv[])
{
    ...
    while ((c=pgetopt(argc, argv, "hHf:v")) NEQ EOF) {
        switch (c) {
         ...
         case 'v':
            printf("Portable MASTER version %u.%u\n", PMASTER_VERSION_MAJOR, PMASTER_VERSION_MINOR);
            exit(0);
        }
    }
...
main(int argc, char *argv[])
{
    ...
    for (cargo=0; cargo<4; cargo++) {
        Info("    %10s: %u-%u KT in %u%% of the planets\n"
             "            %u-%u KT in %u%% of the planets",
                lCargoNames[cargo], cfg->mPlanetCoreRangesUsual[cargo],
    ...
    for (race = 1; race <= 11; race++) {
        if (! cfg->mRaces[race]) continue;
        if (! strcmp(cfg->mPasswords[race], "none")) continue;
...
static void doPlanetRandomize(void)
{
    ...
    /* First, change the relative ratios of native types and governments
       into a CDF */
    ...
    for (lCount=0; lCount<9; lCount++) {
        lNatTypeCDF[lCount] = cfg->mNativeTypeFrequencies[lCount]/(float)lNatTypeSum;
        lNatGovCDF[lCount] = cfg->mNativeGovFrequencies[lCount]/(float)lNatGovSum;
        if (lCount > 0) {
    ...
    Info("Natives will appear on %u%% of the planets.", cfg->mNativesOnPlanetFrequency);

    Info("Percentage of native types and governments:");
    for (lCount=0; lCount < 9; lCount++) {
        Info("    %10s: %5.1f%%    %14s: %5.1f%%",
                lNatTypeName[lCount],
                100*(lNatTypeCDF[lCount] - (lCount EQ 0 ? 0 : lNatTypeCDF[lCount-1])),
                lNatGovName[lCount],
                100*(lNatGovCDF[lCount] - (lCount EQ 0 ? 0 : lNatGovCDF[lCount-1])));
    }
    ...
    for (planet = 1; planet <= PLANET_NR; planet++) {
        ...
        if (RandomRange(100) < cfg->mNativesOnPlanetFrequency) {
            ...
            lClans = (Uns32)RandomRange((Uns16) lRange)*10UL
...
static void doHomeworlds(void)
{
    ...
    float angle;
    Uns16 planet;
    Uns16 assigned[12], totalAssigned;
    ...
    totalAssigned=0;
    for (index = 1;
         index <= 11 AND totalAssigned < gNumRaces;
         index++, angle += (2*M_PI/gNumRaces)) {
        if (cfg->mHomeworldsRadius EQ 0) {
            if (! cfg->mRaces[index]) continue;
...
static void doFreeShips(void)
{
    ...
    for (hullNumber = 1; hullNumber <= 2; hullNumber++) {
        for (race=1; race <= 11; race++) {
            if (! cfg->mRaces[race]) continue;
            if (! cfg->mTwoFreeShips[race]) continue;
...
static int nearestPlanetCompare(const void *p1, const void *p2)
{
...
static Uns16 *findNearestPlanets(Uns16 pPlanet)
{
    static Uns16 *lPlanets = 0;
    PlanetDist_Struct *lDists = 0;
    Uns16 lX, lY, lPlanet, lTotalPlanets;
    Int16 lDX, lDY;
    ...
    lX = PlanetLocationX(pPlanet); lY = PlanetLocationY(pPlanet);
...
static void doAshesScenario(void)
{
    ...
    /* Put some random minerals on planets, add natives, set
       temperature, etc. */
    doPlanetRandomize();
    ...
    for (lPlanets=0; *lClosePlanets AND lPlanets <= cfg->mOldEmpirePlanets;
        ...
        /* lPlanets==0 is the homeworld, lPlanets==1 is the next closest
           planet. */
        if (lPlanets EQ 1) {
...
static void copyPlanetToPlanet(Uns16 pSrc, Uns16 pDst)
{
...
static void doDisunitedScenario(void)
{
    ...
    /* Now compute the maximum number of owned planets any one race has.
       Only need to do this if we're setting each race's initial conditions
       to be the same. */
