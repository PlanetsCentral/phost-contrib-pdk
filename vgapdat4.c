...
static void FreeMines(void)
{
    MemFree(gMinesPtr); gMinesPtr = 0;
}

static void InitMines(void)
{
    if (gMinesPtr EQ 0) {
        gMinesPtr = (MineField_Struct *)MemCalloc(MINE_NR+1, sizeof(MineField_Struct));
...
/* Minefields, unlike planets, ships, and bases, are stored in memory
 ...
 MineField_Struct* GetMinefield(Uns16 pID)
{
    InitMines();
 ...
 void PutMinefield(Uns16 pID, MineField_Struct *pMine)
{
    InitMines();
...
Uns16 CreateMinefield(Uns16 pXloc, Uns16 pYloc, RaceType_Def pRace,
...
IO_Def Read_Mines_File(void)
{
	...
	if ((lMineFile = OpenInputFile(MINE_FILE, GAME_DIR_ONLY)) NEQ NULL) {
...
#ifdef __MSDOS__
		if (500 NEQ fread(gMinesPtr+1, sizeof(MineField_Struct), 500, lMineFile)) {
			Error("Can't read file '%s'", MINE_FILE);
			lError=IO_FAILURE;
		}
#else
		for (lMine=1; lMine <= 500; lMine++) {
			if (! DOSReadStruct(MineStruct_Convert,
								NumMineStruct_Convert,
								gMinesPtr+lMine,
								lMineFile)) {
				Error("Can't read file '%s'", MINE_FILE);
				lError=IO_FAILURE;
        ...
        /* ! Note: if extmines.hst can't be found then we have no error.
           It probably means that the host data was generated by an earlier
           version of PHOST or by HOST. */
	if ((lMineFile = OpenInputFile(MINE_EXT_FILE, GAME_DIR_ONLY + NO_MISSING_ERROR)) NEQ NULL) {
        int lMine;

        /* read extra mines */
#ifdef __MSDOS__
		if ((MINE_NR - 500) NEQ fread(gMinesPtr+1, sizeof(MineField_Struct), (MINE_NR - 500), lMineFile)) {
...
IO_Def Write_Mines_File(void)
{
	...
	if ((lMineFile = OpenOutputFile(MINE_FILE, GAME_DIR_ONLY)) NEQ NULL) {
#ifdef __MSDOS__
		if (MINE_NR NEQ fwrite(gMinesPtr+1, sizeof(MineField_Struct), 500, lMineFile)) {
		...
		int lMine;
		for (lMine=1; lMine <= 500; lMine++) {
			if (! DOSWriteStruct(MineStruct_Convert,
								 NumMineStruct_Convert,
								 gMinesPtr+lMine,
								 lMineFile)) {
				Error("Can't write file '%s'", MINE_FILE);
...
#ifdef __MSDOS__
		if (MINE_NR NEQ fwrite(gMinesPtr+501, sizeof(MineField_Struct), MINE_NR-500, lMineFile)) {
...
void WrapMineLocation(Uns16 pID)
{
	MineField_Struct *lMine = GetMinefield(pID);
