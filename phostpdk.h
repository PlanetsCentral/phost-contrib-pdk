...
typedef enum {
    COMBAT_VICTOR,          /* The ship won the battle (i.e., survived) */
    COMBAT_CAPTURED,        /* The ship was captured by the opponent */
                            /* Doesn't apply to planets */
    COMBAT_DESTROYED,       /* The ship was destroyed */
                            /* Applies to planets */
    COMBAT_NOAMMO           /* Ship has no beams, no torps, no fighters
                               but has not been destroyed or captured */
} CombatResult_Def;
...
typedef struct {
	...
	Boolean  AllowMoreThan50Targets[12]; /* allow target.dat part of .RST file to have >50 targets */
        Boolean  AllowMoreThan500Minefields[12]; /* allow more than 500 minefields */
	...
	Boolean  CPEnableBigTargets;	/* Enable 'bigtargets' command */
        Uns16    CPNumMinefields;  /* max number of minefields, must be between 1 and 10000 */
...
typedef struct {
    ...
    Uns16 mPRange;   /* range at which UFO can be seen from a planet */
    Uns16 mSRange;   /* range at which UFO can be seen from a ship */
    Uns16 mRadius;   /* the radius of the UFO object */
    Uns16 mParentID; /* ID of the program that maintains this UFO */
} UFO_Struct;
...
extern Boolean IsAllyOfferConditional(RaceType_Def pPlayer, RaceType_Def pRace,
                                        AllianceLevel_Def pLevel);
...
typedef struct {
           /* The following fields will not be changed by Combat()
              They must be filled in with appropriate values by
              the code calling Combat(). */
    ...
    Uns16        NumBays;    /* number of fighter bays */
            ...
            /* The following fields WILL be changed by Combat(). Initial
               values for these fields must be filled in before calling
               Combat(). */

    Uns16        Crew;         /* Number of crew members (irrelevant for planets) */
    ...
    Uns16        NumTorps;     /* Number of torps */
            ...
            /* The following fields will be filled in by Combat(). It is
               not necessary to fill them in prior to calling Combat(). */
...
extern void DrawFighterHit(Uns16 pShip, Uns16 pFighter, Int32 pPos, Int32 pEnemyPos);
extern void DrawFighterHitFighter(Int32 pPos, Uns16 pFighter1, Uns16 pFighter2);
...
extern void DrawShipOwner(Uns16 pShip, const char *pRaceName, Boolean pIsPlanet);
...
extern Boolean IsDistanceLTRadius(Int16 pX1, Int16 pY1,
...
extern Uns16 GetNumMinefields(void);             /* return the maximum number of minefields */
