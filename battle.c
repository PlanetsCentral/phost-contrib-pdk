...
static const Uns16 MIN_TUBE_CHARGE              = 1000;     /* tubes only fire when fully charged */
...
static Uns16 BeamHitOdds(Uns16 pBeamTech, const CombatConfig_Struct *pConfig)
{
    Int16 lOdds;

    passert(pConfig NEQ 0);

    lOdds = (Int16) pConfig->BeamHitOdds +
            (Int16) (
        ((Int32)BeamKillPower(pBeamTech)+(Int32)BeamDestructivePower(pBeamTech))
...
static Uns16 TorpHitOdds(Uns16 pTorpTech, const CombatConfig_Struct *pConfig)
{
    Int16 lOdds;

    passert(pConfig NEQ 0);

    lOdds = (Int16) pConfig->TorpHitOdds +
            (Int16) (
        ((Int32)TorpKillPower(pTorpTech)+(Int32)TorpDestructivePower(pTorpTech))
...
static Uns16 BayRechargeRate(Uns16 pNumBays, const CombatConfig_Struct *pConfig)
{
    Int16 lRate;

    passert(pConfig NEQ 0);

    lRate = (Int16) pConfig->BayRechargeRate +
...
static Uns16 BeamRechargeRate(Uns16 pBeamTech, const CombatConfig_Struct *pConfig)
{
    Int16 lRate;

    passert(pConfig NEQ 0);

    lRate = (Int16) pConfig->BeamRechargeRate +
            (Int16) (
        (Int32)(BeamKillPower(pBeamTech)+BeamDestructivePower(pBeamTech))
         * (Int32)(pConfig->BeamRechargeBonus)
         / 100L
                    );
    lRate = max(lRate, 1);
    return (Uns16) lRate;
...
static Uns16 TubeRechargeRate(Uns16 pTubeTech, const CombatConfig_Struct *pConfig)
{
    Int16 lRate;

    passert(pConfig NEQ 0);

    lRate = (Int16) pConfig->TubeRechargeRate +
            (Int16) (
        (Int32)(TorpKillPower(pTubeTech)+TorpDestructivePower(pTubeTech))
         * (Int32)(pConfig->TubeRechargeBonus)
         / 100L
                    );
    lRate = max(lRate, 1);
    return (Uns16) lRate;
...
static double ShieldDamage(Uns16 pBoom, Uns16 pMass, const CombatConfig_Struct *pConfig)
{
    ...
    /* Limiting for sanity */
    if (lShieldDamage > 10000) lShieldDamage = 10000;
...
static double HullDamage(Uns16 pBoom, Uns16 pMass, const CombatConfig_Struct *pConfig)
{
    double lDamage;
    ...
    lDamage =            lDamage
              * (double) (pConfig->HullDamageScaling)
              / (double) (pMass+1);

    /* Limiting for sanity */
    if (lDamage > 10000) lDamage = 10000;
...
static double CrewKilled(Uns16 pKill, Uns16 pMass, const CombatConfig_Struct *pConfig)
{
    double lCrewKill;

    passert(pConfig NEQ 0);

    lCrewKill =   (double) pKill
                * (double) (pConfig->CrewKillScaling)
                / (double) (pMass + 1);
...
static void doBayRecharge(void)
{
    Uns16 lCount;
    Uns16 lBay;
    Uns16 lBayLimit;
...
static void doBeamRecharge(void)
{
    Uns16 lCount;
    Uns16 lBeamLimit;
    ...
    for (lCount=0; lCount<2; lCount++) {
        ...
        for (lBeam = 0; lBeam < lBeamLimit; lBeam++) {
            if (lCurrPtr->mBeamCharge[lBeam] < 1000) {
                lCurrPtr->mBeamCharge[lBeam] += RandomRange(lCurrPtr->mBeamChargeRate);
...
static void doTubeRecharge(void)
{
    Uns16 lCount;
    Uns16 lTubeLimit;
    ...
    for (lCount=0; lCount<2; lCount++) {
        ...
        for (lTube = 0; lTube < lTubeLimit; lTube++) {
            if (lCurrPtr->mTubeCharge[lTube] < 1000) {
                lCurrPtr->mTubeCharge[lTube] += RandomRange(lCurrPtr->mTubeChargeRate);
...
static void doFighterLaunch(void)
{
    Uns16 lCount;
    Uns16 lFighter;
    Uns16 lBay;
    ...
    for (lCount=0; lCount<2; lCount++) {
        ...
        lBayLimit = lCurrPtr->mInfo->NumBays;
        for (lBay = 0; lBay < lBayLimit; lBay++) {
            if (lCurrPtr->mBayCharge[lBay] >= 1000) break;
        }
        if (lBay >= lBayLimit) continue;
        ...
        for (lFighter=0; lFighter < MAX_FIGHTER_OUT(lCount); lFighter++) {
            if (lCurrPtr->mFStatus[lFighter] EQ F_IDLE) {
                ...
                lCurrPtr->mNextLaunchTick = gBattleTick + TICKS_PER_FIGHTER_LAUNCH(lCount);
...
static void doMoveFighters(void)
{
    Uns16 lCount;
    Uns16 lFighter;
    Uns16 lRemaining;
    ...
    for (lCount=0; lCount<2; lCount++) {
        ...
        if ((lRemaining=lCurrPtr->mFLaunched) EQ 0) continue;
        ...
        for (lFighter=0; lRemaining > 0 AND lFighter < MAX_FIGHTER_OUT(lCount);
            ...
            switch (lCurrPtr->mFStatus[lFighter]) {
             ...
             case F_ATTACKING:
                /* Erase the fighter at the old position, update the position
             ...
             case F_RETURNING:
                /* Same concept as above but check for fighter travelling
                   back behind our ship. Once the fighter crosses our ship
                   position, bring it back into the ship */
...
static Boolean doDamageShip(Uns16 pShip, Uns16 pKill, Uns16 pBoom)
{
    /* This variable indicates how much of the kill and destructive power
       actually causes damage as opposed to just lowering shields. It is
       100 if the ship has no shields, 0 if the hit hasn't lowered the
       shields yet, or something in between if the shields were lowered
       by this hit but there's hit power left over */
    Uns16 lDamagePercent = 100;
    double lCrewKill;
    BattleStatus_Struct *lShipPtr = gStatus[pShip];

    if (pKill EQ 0) pKill=1;        /* Bad things happen if these are 0 */
    if (pBoom EQ 0) pBoom=1;
    ...
    if (lShipPtr->mShields > 0) {
        ...
        if (lShieldDamage >= lShipPtr->mShields) {
            /* Pro-rate the amount of oomph from the blast that is left over
               to do damage */
    ...
    if (lDamagePercent > 0) {
        ...
        if ( (     (lDamage + lShipPtr->mDamage >= 99.5)
              ...
              OR
             (lDamage + lShipPtr->mDamage >= 149.5)
           ) {
    ...
    if (lShipPtr->mInfo->IsPlanet) return False;
...
static void doFighterFighterHits(void)
{
    Uns16 lFighter, lFighter2;
    Int16 lPos;
    Int16 lQuantPos1[MAX_FIGHTER_LIMIT], lQuantPos2[MAX_FIGHTER_LIMIT];
    ...
    /* We do not allow the case of both fighters firing at each
       other. This makes 1-hit-2 or 2-hit-1 determination easier
    ...
    } else return; /* means that lP1==lP2==0 or 100...nuthin' to do in this routine */
    ...
    /* Perform position quantization ahead of time to save mucho time
       later. Each element of lQuantPos indicates the quantized position
       of the corresponding fighter. If the fighter is idle, the position
       is given as -32767. */
    ...
    /* For each fighter belonging to ship 0, find a fighter from ship 1 that
       occupies the same point in space. If such fighters are found, then
       a coin is flipped to decide which fighter is destroyed. */

    for (lFighter = 0; lFighter < lMaxFighters; lFighter++) {
        if (lQuantPos1[lFighter] EQ -32767) continue;

        lPos = lQuantPos1[lFighter];

        /* With random probability, a fighter-to-fighter hit does not
           succeed so there is no point in searching for an enemy fighter
           at the same position. In PHOST 3 we must take into account
           differing probabilities. We computed hit probability above
           according to model that a hit occurs when A fires OR B
           fires but NOT BOTH. */
        if (RandomRange(100) >= lHitProbability) continue;
        ...
        for (lFighter2 = 0; lFighter2 < lMaxFighters; lFighter2++) {
            if (lPos EQ lQuantPos2[lFighter2]) {
                /* Do the kill. Decide who loses. With asymmetrical
                   probabilities, we must weight one side more (or
                   less) than the other, as necessary. This was computed
                   above as l2Hit1Probability. */
                ...
                DrawFighterHitFighter(gStatus[0]->mFPos[lFighter], lFighter, lFighter2);
                ...
                /* If it's the second fighter that lost, set its quantized
                   position to -32767. We don't have to worry if the first
                   fighter lost since we're not looking at it again. */
                if (lLoser) {
...
static Boolean doFighterDamage(void)
{
    Uns16 lCount;
    Uns16 lFighter;
    Uns16 lRemaining;
    ...
    for (lCount=0; lCount<2; lCount++) {
        ...
        for (lFighter=0; lRemaining > 0 AND lFighter < MAX_FIGHTER_OUT(lCount); lFighter++) {
            switch (lCurrPtr->mFStatus[lFighter]) {
             case F_IDLE: break;
             case F_RETURNING: lRemaining--; break;
             case F_ATTACKING:
                lRemaining--;
                ...
                if (labs(lOtherPtr->mPos - lCurrPtr->mFPos[lFighter])
                    ...
                    DrawFighterHit(lCount, lFighter, lCurrPtr->mFPos[lFighter], lOtherPtr->mPos);
...
static Boolean doTorpLaunch(void)
{
    Uns16 lCount;
    Uns32 lDistance=0;
    ...
    for (lCount = 0; lCount < 2; lCount++) {
        ...
        if (lCount EQ 0) {
            ...
            if (lDistance > MIN_TORP_SHIP_HIT_DISTANCE(0)) continue;
        } else {
            if (lCurrPtr->mInfo->IsPlanet) {
                if (lDistance > MIN_TORP_PLANET_HIT_DISTANCE(1)) return False;
            } else if (lDistance > MIN_TORP_SHIP_HIT_DISTANCE(1)) return False;
        }

        if (lCurrPtr->mInfo->NumTorps EQ 0) continue;
...
static Boolean doBeamFire(void)
{
    Uns16 lCount;
    ...
    for (lCount = 0; lCount < 2; lCount++) {
        ...
        for (lBeam = 0; lBeam < lBeamLimit; lBeam++) {
            ...
            /* If beam meets minimum charge level for fighters, try to fire
               on a fighter. Ships must be close enough to fighters for
               this to happen. Find closest other fighter to see if this
               is the case. */
            ...
            if (lCurrPtr->mBeamCharge[lBeam]
                ...
                /* First, look for attacking fighters. In fact, look for
                   the CLOSEST attacking fighter. */
                ...
                for (lFighter = 0; lFighter < MAX_FIGHTER_OUT(1-lCount); lFighter++) {
                    /* In any case, we fire on the closest fighter. The only
                       decision is whether we fire on the closest attacking
                       fighter, or whether it's just the closest fighter,
                       period. */
                ...
                if (lClosest >= 0) {
                    ...
                    if (lMissed) {
                        DrawBeamMissedFighter(lCount, lBeam, lCurrPtr->mPos, lClosest, lOtherPtr->mFPos[lClosest]);
                    } else {
                        DrawBeamHitFighter(lCount, lBeam, lCurrPtr->mPos, lClosest, lOtherPtr->mFPos[lClosest]);
                        if (lOtherPtr->mFStatus[lClosest] EQ F_ATTACKING) {
                            DrawEraseFighterAttack(1-lCount, lClosest, lOtherPtr->mFPos[lClosest]);
                        } else {
                            DrawEraseFighterReturn(1-lCount, lClosest, lOtherPtr->mFPos[lClosest]);
                        }
                    }
                ...
                if (gConfigInfo->FireOnAttackFighters) {
                    for (lFighter = 0; lFighter < MAX_FIGHTER_OUT(1-lCount); lFighter++) {
                        if (lOtherPtr->mFStatus[lFighter] EQ F_RETURNING) {
                            if (lMissed) {  /* NEW */
                                DrawBeamMissedFighter(lCount, lBeam, lCurrPtr->mPos, lFighter, lOtherPtr->mFPos[lFighter]);
                            } else {
                                DrawBeamHitFighter(lCount, lBeam, lCurrPtr->mPos, lFighter, lOtherPtr->mFPos[lFighter]);
                                DrawEraseFighterReturn(1-lCount, lFighter, lOtherPtr->mFPos[lFighter]);
                            }
            ...
            /* Didn't fire on a fighter. Let's try firing on the ship. However,
               if there are enemy fighters launched, do not waste beam weapons
               firing on a ship. Save them up until they reach the charge
               level necessary to fire on fighters. */
            if (lOtherPtr->mFLaunched > 0) continue;
...
static void doMoveShips(void)
{
    ...
    for (lCount=0; lCount < 2; lCount++) {
        ...
        if (lCount EQ 0) {
            ...
            if (lDistance <= STANDOFF_DISTANCE) return;
        }
...
static Boolean checkShipHasAmmo(Uns16 pShip)
{
...
static Boolean DoBattleTick(BattleStatus_Struct *pS1, BattleStatus_Struct *pS2)
{
    gStatus[0] = pS1;
    gStatus[1] = pS2;
    ...
    /* Violent stuff. These routines return True if one of the ships
       has been destroyed or captured. */

    if (doFighterDamage()) return True;
    if (doTorpLaunch()) return True;
    if (doBeamFire()) return True;
...
static Boolean DisplayBattle(BattleStatus_Struct *pS1, BattleStatus_Struct *pS2)
{
    Boolean lBattleDone;
    gBattleTick = 0;
...
static void getPlayerCombatConfig(Uns16 pPlayer, CombatConfig_Struct *pConfig)
{
    /* If player is out of range then this is either a Jumpgate alien or an
       unassigned race(?). In either case, make it player 1 by default.
       Why not. Also, do the same for version 2.x or 1.x VCR's. */
    if (   (pPlayer < 1)
...
void Combat(Combat_Struct *pShip1, Combat_Struct *pShip2)
{
    ...
    if (gInitialStatus[0] EQ 0) {
        for (lCount=0; lCount<2; lCount++)
            gInitialStatus[lCount] = (BattleStatus_Struct *)MemAlloc(sizeof(BattleStatus_Struct));
    }

    /* Allocate combat config structures if necessary */
    if (gCombatConfig[0] EQ 0) {
        for (lCount=0; lCount < 2; lCount++) {
            gCombatConfig[lCount] = (CombatConfig_Struct *)MemAlloc(sizeof(CombatConfig_Struct));
        }
    }
    ...
    for (lCount=0; lCount<2; lCount++) {
        ...
        DrawShipShields(lCount, STARTING_POSITION[lCount], lCurrPtr->mInfo->Shields);
        ...
        if (lCurrPtr->mInfo->NumBeams > 0) {
            ...
            lCurrPtr->mBeamHitOdds = BeamHitOdds(lCurrPtr->mInfo->BeamTech,
                                                        gCombatConfig[lCount]);
            lCurrPtr->mBeamChargeRate = BeamRechargeRate(lCurrPtr->mInfo->BeamTech,
                                                        gCombatConfig[lCount]);
        }

        if (lCurrPtr->mInfo->NumTubes > 0) {
            /* Compute the torp recharge rate and hit probability */
            lCurrPtr->mTorpHitOdds = TorpHitOdds(lCurrPtr->mInfo->TorpTech,
                                                    gCombatConfig[lCount]);
            lCurrPtr->mTubeChargeRate = TubeRechargeRate(lCurrPtr->mInfo->TorpTech,
                                                    gCombatConfig[lCount]);
        ...
        if (lCurrPtr->mInfo->NumBays > 0) {
            ...
            lCurrPtr->mBayChargeRate = BayRechargeRate(lCurrPtr->mInfo->NumBays,
                                                        gCombatConfig[lCount]);
        ...
        /* Charge tubes */
        /* For some reason, this code is compiled incorrectly by the
           Borland C++ 3.1 and 4.5 compilers with the -Ol optimization
           option. DON'T USE IT! */
        {
            Uns16 lTube;
        ...
        DrawInitBeams(lCount,lCurrPtr->mInfo->NumBeams,lCurrPtr->mBeamCharge[0]);
        DrawInitTubes(lCount,lCurrPtr->mInfo->NumTubes,lCurrPtr->mTubeCharge[0]);
    }
    ...
    for (lCount=0; lCount < 2; lCount++) {
        ...
        if (lCurrPtr->mDamage > 0) lCurrPtr->mDamage += 0.5;
        if (lCurrPtr->mShields > 0) lCurrPtr->mShields += 0.5;
        if (lCurrPtr->mCrew > 0) lCurrPtr->mCrew += 0.5;
        ...
        if (lCurrPtr->mDamage >= 100) {
            ...
            /* If the ship was captured, leave the status as captured since
               this is more important than whether or not it has ammo. Otherwise,
               if this ship thinks it's the victor, downgrade its result
               to NOAMMO if warranted */
