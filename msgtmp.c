...
Boolean
WriteAUXHOSTMessage(RaceType_Def pRace, const char *pMessage)
{
  Uns16 lLength;
  char *lStr = 0;
  Uns16 lCount;
  ...
  if (gPointerFile EQ 0) {
    ...
    gPointerFile =
          OpenUpdateFile(TMP_MSG_POINTER_FILE, GAME_DIR_ONLY | REWRITE_MODE);
    rewind(gPointerFile);

    if (1 NEQ fread(&gNumExtMessages, sizeof(gNumExtMessages), 1,
                gPointerFile)) {
      /* Must have no messages */
      gNumExtMessages = 0;
      rewind(gPointerFile);
      (void) fwrite(&gNumExtMessages, sizeof(gNumExtMessages), 1,
            gPointerFile);
    }
...
#ifdef __MSDOS__
  if (1 NEQ fwrite(&lStruct, sizeof(lStruct), 1, gPointerFile))
    goto bad_ext_write;
#else
  if (!DOSWriteStruct(ExtMessageStruct_Convert, NumExtMessageStruct_Convert,
              &lStruct, gPointerFile))
    goto bad_ext_write;
#endif
