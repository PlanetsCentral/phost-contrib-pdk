...
static void FreePlanets(void)
{
    MemFree(gAllPlanets); gAllPlanets = 0;
}

static void InitPlanets(void)
{
    if (gAllPlanets EQ 0) {
        gAllPlanets = (Planet_Struct *)MemCalloc(PLANET_NR+1, sizeof(Planet_Struct));
...
--------------------------------------------------------------------------*/
 ...
 Planet_Struct* GetPlanet(Uns16 pID)
{
    InitPlanets();
...
/* _All_ planets should exist */
 ...
 void PutPlanet(Uns16 pID, Planet_Struct* pPlanet)
{
    InitPlanets();
...
{
	if (pType EQ COLONISTS) pAmount /= 100;
 ...
 void PutPlanetBuildBase(Uns16 pID, Boolean pBuild)
{
 ...
 void ChangePlanetOwner(Uns16 pID, RaceType_Def pNewOwner)
{
	PutPlanetOwner(pID, pNewOwner);
	if (IsBasePresent(pID)) {
		ClearBaseBuildOrder(pID);
		if (pNewOwner NEQ NoOwner) PutBaseOwner(pID, pNewOwner);
		ClearBaseHulls(pID);
	}
}
...
const char *PlanetTempString(Uns16 pPlanet)
{
	...
	if (lTemp < 15) return "Arctic World";
	else if (lTemp < 40) return "Temperate - cool";
	else if (lTemp < 65) return "Temperate - warm";
	else if (lTemp < 85) return "Tropical World";
	else return "Desert World";
}

IO_Def Read_Planets_File(Int16* pControl)
  {
    FILE           *lPlanetFile;
    Int16          i;
    ...
    if ((lPlanetFile = OpenInputFile(PLANET_FILE, GAME_DIR_ONLY | NO_MISSING_ERROR)) NEQ NULL) {
        if (1 NEQ fread(pControl, sizeof(Uns16), 1, lPlanetFile)) { /* first 2 bytes in pdata.hst */
        ...
        for (i=1; i<=PLANET_NR; i++ )
        {
			...
			if (! DOSReadStruct(PlanetStruct_Convert,
								NumPlanetStruct_Convert,
								&lPlanet,
								lPlanetFile)) {
#endif
                Error("Can't read file '%s'", PLANET_FILE);
...
IO_Def Write_Planets_File(Int16 pControl)
{
    FILE             *lPlanetFile;
    Int16            i;
    IO_Def           lError = IO_SUCCESS;
...
#else
			if (! DOSWriteStruct(PlanetStruct_Convert,
								 NumPlanetStruct_Convert,
								 IsPlanetExist(i) ? GetPlanet(i) : &lPlanet,
								 lPlanetFile)) {
#endif
                Error("Can't write to file '%s'", PLANET_FILE);
                lError=IO_FAILURE;
