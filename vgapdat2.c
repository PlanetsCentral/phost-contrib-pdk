...
static void FreeShips(void)
{
    MemFree(gAllShips); gAllShips = 0;
}

static void InitShips(void)
{
    if (gAllShips EQ 0) {
 ...
 Ship_Struct* GetShip(Uns16 pID)
{
 ...
 void PutShip(Uns16 pID, Ship_Struct* pShip)
{
    InitShips();
...
static Uns16 ShipLocation(Uns16 pID, XYComponent_Def pIndex)
{
...
static Uns16 ShipWaypoint(Uns16 pID, XYComponent_Def pIndex)
{
	passert( (pIndex EQ 0) OR (pIndex EQ 1) );
...
static void PutShipWaypoint(Uns16 pID, XYComponent_Def pIndex, Uns16 pWaypoint)
{
	passert( (pIndex EQ 0) OR (pIndex EQ 1) );

	/* NEW */
	GetShip(pID)->Waypoint[pIndex]=(Int16)pWaypoint - (Int16)ShipLocation(pID, pIndex);
}
...
Uns16 ShipCargo(Uns16 pID, CargoType_Def pType)
{
	...
	case TRITANIUM	:
	case DURANIUM	:
	case MOLYBDENUM : return(GetShip(pID)->Cargo[pType]);
	case SUPPLIES	: return(GetShip(pID)->Cargo[4]);
	case CREDITS	: return(GetShip(pID)->Credits);
	case COLONISTS	: return(GetShip(pID)->Colonists);	/* CLANS!!! */
	default: passert(0);
	}
	return(0);
}
...
void PutShipCargo(Uns16 pID, CargoType_Def pType, Uns16 pCargo)
{
	...
	case TRITANIUM	:
	case DURANIUM	:
	case MOLYBDENUM : GetShip(pID)->Cargo[pType]=pCargo;
					  break;
	case SUPPLIES	: GetShip(pID)->Cargo[4]=pCargo;
					  break;
	case CREDITS	: GetShip(pID)->Credits=pCargo;
					  break;
	case COLONISTS	: GetShip(pID)->Colonists=pCargo;	/* CLANS!!! */
					  break;
	}
}
...
IO_Def Read_Ships_File(Int16* pControl)
{
    FILE         *lShipFile;
    Int16        i;
    ...
    if ((lShipFile = OpenInputFile(SHIP_FILE, GAME_DIR_ONLY | NO_MISSING_ERROR)) NEQ NULL) {
        ...
        for (i=1; i<=SHIP_NR; i++ )
        {
			...
			if (! DOSReadStruct(ShipStruct_Convert,
								NumShipStruct_Convert,
								&lShip,
								lShipFile)) {
#endif
                Error("Can't read file '%s'", SHIP_FILE);
...
IO_Def Write_Ships_File(Int16 pControl)
{
    FILE           *lShipFile;
    Int16          i;
    IO_Def         lError = IO_SUCCESS;
...
#else
			if (! DOSWriteStruct(ShipStruct_Convert,
								 NumShipStruct_Convert,
								 IsShipExist(i) ? GetShip(i) : &lShip,
								 lShipFile)) {
#endif
                Error("Can't write to file '%s'", SHIP_FILE);
                lError=IO_FAILURE;
...
void WrapShipLocation(Uns16 pID)
{
	Int16 lLocation, lWrap, lWaypoint;
	Ship_Struct* lShip = GetShip(pID);
