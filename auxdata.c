...
static Boolean ReadScanInfo(FILE *pInfile)
{
    InitAuxData();
...
static void FreeCLOAKC(void)
{
  MemFree(gCombatShields); gCombatShields = 0;
}
...
Boolean IsShipCloaked(Uns16 pShip)
{
    Uns16 lMajor, lMinor;
    Boolean lIsAuxdataPresent = GameFilesVersion(&lMajor, &lMinor);

    /* Preconditions for valid cloak info:
       (a) No auxdata (HOST) and CLOAKC.HST was loaded
       OR (b) auxdata present (PHOST) and version > 2.10
    */
...
#if 0
void MarkShipCloaking(Uns16 pShip, Boolean pIsCloaking)
{
    ...
    /* If we want the ship to be uncloaked for all phases, mark the cloak
       as having failed so we don't re-cloak in a later phase */
    if (pIsCloaking EQ False) gShipInfo[pShip].mCloakHasFailed = True;
}
#endif
...
Uns16 BuildQueuePeek(BaseOrder_Struct *pOrder)
{
	...
	if (gNumBaseOrders EQ 0) return 0;
...
static const BaseOrder_Struct *BuildQueueAccess(Uns16 pMember)
{
    InitAuxData();

    if (pMember >= gNumBaseOrders) return 0;

	return gBaseOrder + pMember;
}

static void invalidateOrder(Uns16 pOrder)
{
...
void BuildQueuePush(BaseOrder_Struct *pOrderPtr)
{
    ...
    for (lCount = 0; lCount < gNumBaseOrders; lCount++) {
        if (gBaseOrder[lCount].mBase EQ pOrderPtr->mBase) break;
    }
    ...
    /* Now, this base is already in the queue. Is the order the same or is
       it different? Can't just compare BaseOrder structures because
       priorities may well be different. Compare BuildOrder structures
    ...
    /* The orders are different. Subtract penalty priority from the
       order */
...
void BuildQueueInvalidate(Uns16 pBase)
{
    ...
    /* Try to find the given base in the queue */
	for (lCount = 0; lCount < gNumBaseOrders; lCount++) {
		if (gBaseOrder[lCount].mBase EQ pBase) break;
	}
	if (lCount >= gNumBaseOrders) return;
...
static void InitBuildQueue(void)
{
	gBaseOrder = (BaseOrder_Struct *)MemCalloc(PLANET_NR, sizeof(BaseOrder_Struct));
}

static void FreeBuildQueue(void)
{
	MemFree(gBaseOrder); gBaseOrder = 0;
}
...
static Boolean ReadBuildQueue(FILE *pFile)
{
	passert(gBaseOrder NEQ 0);

    {
#ifdef __MSDOS__
        if (PLANET_NR NEQ fread(gBaseOrder, sizeof(BaseOrder_Struct), PLANET_NR, pFile))
            goto bad_read;
#else
        Uns16 lPlanet;
        for (lPlanet=0; lPlanet < PLANET_NR; lPlanet++) {
            if (! DOSReadStruct(BaseOrderStruct_Convert,
	...
	/* Count the number of current base orders by searching over all orders
	   and stopping when the base number is 0 */
...
static Boolean WriteBuildQueue(FILE *pFile)
{
	passert(gBaseOrder NEQ 0);

#ifdef __MSDOS__
    if (PLANET_NR NEQ fwrite(gBaseOrder, sizeof(BaseOrder_Struct), PLANET_NR, pFile))
        goto bad_write;
#else
    {
        Uns16 lPlanet;
        ...
        for (lPlanet=0; lPlanet < PLANET_NR; lPlanet++) {
            if (! DOSWriteStruct(BaseOrderStruct_Convert,
                             NumBaseOrderStruct_Convert,
                             gBaseOrder+lPlanet,
...
static int prioritySortFunc(const void *p1, const void *p2)
{
...
void SortBuildQueue(void)
{
    ...
    if (gNumBaseOrders)
    qsort(gBaseOrder, gNumBaseOrders, sizeof(gBaseOrder[0]), prioritySortFunc);
}
...
static Boolean ReadAuxVersionInfo(FILE *pInFile)
{
	/*
	...
	*/
...
#ifdef __MSDOS__
    if (1 NEQ fread(&lTurntime, sizeof(lTurntime), 1, pInFile)) goto bad_read;
#else
    if (! DOSReadStruct(TurntimeStruct_Convert,
                            NumTurntimeStruct_Convert,
                            &lTurntime,
                            pInFile)) goto bad_read;
#endif

    /* Check to make sure timestamp matches NEXTTURN.HST. If not, this
       file is stale. */
    ...
    if (1 NEQ fread(gAuxilliaryInfo, 16, 1, pInFile)) goto bad_read;
...
static Boolean WriteAuxVersionInfo(FILE *pOutFile)
{
	Uns8 lVersion[2];	/* Element 0: major, element 1: minor */
...
#ifdef __MSDOS__
    if (1 NEQ fwrite(lTurnPtr, sizeof(*lTurnPtr), 1, pOutFile)) goto bad_write;
#else
    if (! DOSWriteStruct(TurntimeStruct_Convert,
                         NumTurntimeStruct_Convert,
                         lTurnPtr,
                         pOutFile)) goto bad_write;
#endif

    if (1 NEQ fwrite(gAuxilliaryInfo, 16, 1, pOutFile)) goto bad_write;
...
static void FreeBaseNativeInfo(void)
{
	MemFree(gBaseNatives); gBaseNatives = 0;
}

/* File I/O */

static Boolean ReadBaseNativeInfo(FILE *pFile)
{
	passert(gBaseNatives NEQ 0);
	if (1 NEQ fread(gBaseNatives, PLANET_NR+1, 1, pFile)) return False;

	return True;
}

static Boolean WriteBaseNativeInfo(FILE *pFile)
{
	passert(gBaseNatives NEQ 0);
	if (1 NEQ fwrite(gBaseNatives, PLANET_NR+1, 1, pFile)) return False;
...
{
    /* A player allows his ally the given level IF:
          * The level has been configured to be allowed
            AND * Either the conditional bit for this level has not been set
                   OR
                * The ally offers the same level to the player
    */
    InitAuxData();
...
Boolean PlayersAreAllies(RaceType_Def pPlayer, RaceType_Def pRace)
{
    ...
    if (! PlayerIsActive(pPlayer) OR ! PlayerIsActive(pRace)) return False;

	return	   (gAlliances[pPlayer][pRace] & ALLY_OFFER_BIT)
		   AND (gAlliances[pRace][pPlayer] & ALLY_OFFER_BIT);
}

static Boolean ReadAllianceInfo(FILE *pFile)
{
    if (sizeof(Uns16) NEQ fread(gAlliances, (RACE_NR+1)*(RACE_NR+1), sizeof(Uns16), pFile))
        return False;
...
static Boolean WriteAllianceInfo(FILE *pFile)
{
    EndianSwap16(gAlliances, (RACE_NR+1)*(RACE_NR+1));
    if (sizeof(Uns16) NEQ fwrite(gAlliances, (RACE_NR+1)*(RACE_NR+1), sizeof(Uns16), pFile))
        return False;
    EndianSwap16(gAlliances, (RACE_NR+1)*(RACE_NR+1));
...
static Boolean ReadActivityLevel(FILE *pFile)
{
    if ((RACE_NR+1) NEQ fread(gActivityLevel, sizeof(gActivityLevel[0]),
                                        RACE_NR+1, pFile)) return False;
    EndianSwap32(gActivityLevel, RACE_NR+1);

    return True;
}

static Boolean WriteActivityLevel(FILE *pFile)
{
    EndianSwap32(gActivityLevel, RACE_NR+1);
...
static void InitRemoteControl(void)
{
    ...
    gOriginalOwner = (Uns16 *) MemCalloc(SHIP_NR+1, sizeof(gOriginalOwner[0]));
}
...
static void FreeRemoteControl(void)
{
    ...
    MemFree(gRemoteOwner); gRemoteOwner = 0;
    MemFree(gOriginalOwner); gOriginalOwner = 0;
}

static Boolean ReadRemoteControl(FILE *pFile)
{
    passert(gRemoteOwner NEQ 0);
    if ((SHIP_NR+1) NEQ fread(gRemoteOwner, sizeof(gRemoteOwner[0]),
                                        SHIP_NR+1, pFile)) return False;
...
static Boolean WriteRemoteControl(FILE *pFile)
{
    EndianSwap16(gRemoteOwner, SHIP_NR+1);
...
Boolean BeginShipRemoteControl(Uns16 pShipID, Uns16 pRemoteOwner)
{
    ...
    if (gRemoteOwner[pShipID] & 0x8000U) {
        ...
        /* Ship may already be remote controlled. In this case, do not
           override original owner */
...
Boolean EndShipRemoteControl(Uns16 pShipID, Uns16 pRemoteOwner)
{
    ...
    /* We only terminate the remote control if the pRemoteOwner
       parameter matches the current remote controller. */

    if (! IsShipExist(pShipID)) return False;
...
static void HandleMissingAuxdata(void)
{
    InitAuxData();
...
static Boolean ReadAuxData(FILE *pAuxDataFile)
{
    InitAuxData();

    if (! ReadAuxVersionInfo(pAuxDataFile)) goto bad_read;
	if (! ReadBaseNativeInfo(pAuxDataFile)) goto bad_read;
	if (! ReadAllianceInfo(pAuxDataFile)) goto bad_read;
	if (! ReadScanInfo(pAuxDataFile)) goto bad_read;
	if (! ReadBuildQueue(pAuxDataFile)) goto bad_read;
    if (! ReadActivityLevel(pAuxDataFile)) goto bad_read;
    if (! ReadRemoteControl(pAuxDataFile)) goto bad_read;
...
static Boolean WriteAuxData(FILE *pAuxDataFile)
{
    InitAuxData();

    if (gNoAuxData) return True;
	...
	if (! WriteAuxVersionInfo(pAuxDataFile)) goto bad_write;
	if (! WriteBaseNativeInfo(pAuxDataFile)) goto bad_write;
	if (! WriteAllianceInfo(pAuxDataFile)) goto bad_write;
	if (! WriteScanInfo(pAuxDataFile)) goto bad_write;
	if (! WriteBuildQueue(pAuxDataFile)) goto bad_write;
    if (! WriteActivityLevel(pAuxDataFile)) goto bad_write;
    if (! WriteRemoteControl(pAuxDataFile)) goto bad_write;
...
IO_Def Read_AuxData_File(void)
{
    ...
    if ((lAuxDataFile = OpenInputFile(AUXDATA_FILE, GAME_DIR_ONLY | NO_MISSING_ERROR)) NEQ NULL) {
        if (! ReadAuxData(lAuxDataFile)) {
            lError = IO_FAILURE;
        } else gReadAuxData = True;
        fclose(lAuxDataFile);
    } else {
        if (gNonPHOSTWarnings) Warning("Can't find file '%s'", AUXDATA_FILE);
...
IO_Def Write_AuxData_File(void)
{
    FILE    *lAuxDataFile;
    IO_Def  lError = IO_SUCCESS;

    if (! gReadAuxData) return IO_SUCCESS;
...
Boolean WriteCLOAKCFile(void)
{
    Uns16 lShip;
    FILE *lFile = OpenOutputFile("cloakc.hst", GAME_DIR_ONLY);
    Uns16 lStatus;

    /* Info(StringRetrieve(S__Writing_File, gConfigInfo->Language[0]), "CLOAKC.HST"); */
...
void ReadCLOAKCFile(void)
{
    ...
    /* Info(StringRetrieve(S__Reading_File, gConfigInfo->Language[0]), "CLOAKC.HST"); */
    ...
    for (lShip = 1; lShip <= SHIP_NR; lShip++) {
        if (!DOSRead16(&lStatus, 1, lFile)) {
            Error("Unable to read shield info for ship %u in CLOAKC.HST file", lShip);
            return;
        }
