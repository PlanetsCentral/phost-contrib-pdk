...
static Boolean DoAssignment(const char *name, char *val, const char *pInputLine);
static const char *languageName(Language_Def pLanguage);

/* ---------------------------------------------------------------------- */

IO_Def ConfigFileReader(FILE *pInFile, const char *pFileName,
                ...
                configAssignment_Func pAssignFunc)
{
    ...
    while (! feof(pInFile)) {
        ...
        *ep++ = 0;
        ep = SkipWS(ep);
        if (ep EQ 0) {
            /* ErrorExit("%s: expecting value to assign at line %u", pFileName, line); */
        ...
        TrimTrailingWS(p);
        if (*p EQ 0) {
            /* ErrorExit("%s: expecting parameter name to assign to at line %u", pFileName, line); */
            Error("%s: expecting parameter name to assign to at line %u", pFileName, line);
            return IO_FAILURE;
        }
...
IO_Def Read_HConfig_File(void)
{
    ...
    lRes = ConfigFileReader(lConfigFile, CONFIG_FILE, "phost", True, DoAssignment);
...
#if 0
static const char *scoreMethodName(ScoreMethod_Def pMethod)
{
...
DoAssignment(const char *name, char *val, const char *lInputLine)
{
    ...
    if (name EQ 0 OR val EQ 0) return True;
    ...
    if (! gAssigningDefaults AND UserSet[ix]) {
        Error("%s: Parameter '%s' is being assigned twice", CONFIG_FILE, Names[ix]);
        return False;
    }
    ...
    switch (Types[ix]) {
     ...
     case CFType_Language_Def: return readLanguageType(ix, val);
     case CFType_ScoreMethod_Def: return readScoreMethodType(ix, val);
     case CFType_char: return readCharType(ix, val);
     default:
        passert(0);
    }
    ...
    for (i=0; i<n; i++) {
        ...
        if (! lDoMA) {
            ...
            if (uval < 0 OR uval > 65535U)  {
                Error("%s: illegal 16-bit unsigned quantity '%ld'", CONFIG_FILE, uval);
                return 0;
            }
            if (!checkValue(ix, uval)) return 0;
        }
...
readScoreMethodType(Uns16 ix, char *val)
{
    ...
    for (i=0; i<n; i++) {
        ...
        /* The order of methods in the following list is important and
           must match the order of definitions in auxdata.c */
        ...
        *(ScoreMethod_Def *)(Data + Pos[ix] + i*sizeof(ScoreMethod_Def)) = (ScoreMethod_Def)match;
    }
...
readCharType(Uns16 ix, char *val)
{
    ...
    if (strlen(val) > n) {
        Warning("%s: assignment to '%s' exceeds string length", CONFIG_FILE, Names[ix]);
    }
    n = min(n, strlen(val));
